...
if __name__ == "__main__":
    main_folder = "/data/gcs/lungNENomics/work/TestSetImgCarcinoids"
    list_main_folder = os.listdir(main_folder)
    df_main = pd.DataFrame( columns=['tileName', 'R', 'G', 'B'])
    for sample in list_main_folder: 
        try:
            df_main = df_main.append(pick_random_pixels_data("/data/gcs/lungNENomics/work/TestSetImgCarcinoids", sample))
        except:
            print('operation failed for the sample {}'.format(sample))
    df_main.to_csv('RandomSamplingTest.csv')
